...
export function filterSessions(sessions: Session[], filters: FilterState, now: Date): Session[] {
  return sessions.filter((session) => {
    ...
    if (filters.query) {
      ...
      const inSpeakers = session.speakers.some((s) => s.name.toLowerCase().includes(q));
